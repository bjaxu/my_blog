	...
	$(document).ready(function(e) {
		...
		var CalendarHandler = {
			...
			initialize: function() {
				...
				$(".currentItem>a").css("margin-left", ($(".item1").width() - 25) / 2 + "px").css("margin-top", ($(".item1").height() - 25) / 2 + "px");
			...
			CreateCalendar: function(y, m, d) {
				...
				if (nowWeek != 0) {
					...
					for (var i = (lastMonthDaysNub - (nowWeek - 1)); i < lastMonthDaysNub; i++) {
						$dayItem.append("<div class=\"item1 lastItem\"><a style='color:  #d1c7b7 !important'>" + (i + 1) + "</a></div>");
					}
				}

				//生成当月的日期
				for (var i = 0; i < nowDaysNub; i++) {
					if (i == (nowDay - 1)) $dayItem.append("<div class=\"item1 currentItem\"><a>" + (i + 1) + "</a></div>");
					else $dayItem.append("<div class=\"item1\"><a>" + (i + 1) + "</a></div>");
				}
